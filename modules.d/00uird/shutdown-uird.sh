...
[ -f ${SYSMNT}/changes/.savetomodule ] && grep -q . ${SYSMNT}/changes/.savetomodule && SAVETOMODULENAME="$(cat ${SYSMNT}/changes/.savetomodule)"
[ -f ${SYSMNT}/changes/.savetomodule ] && SAVETOMODULE=${SAVETOMODULE-yes}
...
for mntp in $(mount | awk  '{print $3}' | sort -r) ; do
