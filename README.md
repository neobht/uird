...
UIRD - это разновидность initrd, его задача собрать из слоев aufs корневую файловую систему и передать загрузку /sbin/init (systemd) с этой корневой fs.
В основе реализации UIRD лежит набор скриптов инициализации dracut (модули base, kernel-modules),
