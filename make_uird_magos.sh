...
dracut  -N  -f -m "base uird uird-network ntfs kernel-modules"  \
	-d "loop cryptoloop aes-generic aes-i586 pata_acpi ata_generic ahci xhci-hcd \
        ...
        -i initrd / \
        --kernel-cmdline "uird.from=/MagOS,/MagOS-Data uird.ro=*.xzm,*.rom,*.rom.enc,*.pfs,*.sfs uird.rw=*.rwm,*.rwm.enc uird.cp=*.xzm.cp uird.load=* uird.noload=/optional/,/machines/,*/homes/*.xzm,/cache/ uird.machines=/MagOS-Data/machines uird.config=MagOS.ini" \
